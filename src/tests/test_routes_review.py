...
from src.modules.core.application.read_model.review_read_model import ReviewReadModel
...
class TestGetAllReviews:
    def test_returns_list(self, client):
        ...
        with patch("src.modules.core.presentation.control_review.GetAllReviewsHandler") as MockHandler:
...
class TestCreateReview:
    def test_success_returns_201(self, client):
        with patch("src.modules.core.presentation.control_review.CreateReviewHandler") as MockHandler:
            MockHandler.return_value.handle = AsyncMock(return_value=5)
            resp = client.post(
                "/review/create",
    ...
    def test_empty_content_returns_400(self, client):
        with patch("src.modules.core.presentation.control_review.CreateReviewHandler") as MockHandler:
            MockHandler.return_value.handle = AsyncMock(side_effect=EmptyReviewError())
            resp = client.post(
                "/review/create",
...
class TestDeleteReview:
    def test_success_returns_200(self, client):
        with patch("src.modules.core.presentation.control_review.DeleteReviewHandler") as MockHandler:
